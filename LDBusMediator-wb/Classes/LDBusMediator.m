...
+(BOOL)bus_canRouteURL:(nonnull NSURL *)URL{
    ...
    [g_connectorMap enumerateKeysAndObjectsWithOptions:NSEnumerationReverse usingBlock:^(NSString * _Nonnull key, id<LDBusConnectorPrt>  _Nonnull connector, BOOL * _Nonnull stop) {
        if([connector respondsToSelector:@selector(bus_canOpenURL:)]){
            if ([connector bus_canOpenURL:URL]) {
                success = YES;
                *stop = YES;
            }
        }
...
+(BOOL)bus_routeURL:(nonnull NSURL *)URL{
    return [self bus_routeURL:URL withParameters:nil];
}
...
+(BOOL)bus_routeURL:(nonnull NSURL *)URL withParameters:(nullable NSDictionary *)params{
    ...
    [g_connectorMap enumerateKeysAndObjectsWithOptions:NSEnumerationReverse usingBlock:^(NSString * _Nonnull key, id<LDBusConnectorPrt>  _Nonnull connector, BOOL * _Nonnull stop) {
        queryCount++;
        if([connector respondsToSelector:@selector(bus_connectToOpenURL:params:)]){
            id returnObj = [connector bus_connectToOpenURL:URL params:userParams];
...
+(nullable UIViewController *)bus_viewControllerForURL:(nonnull NSURL *)URL{
    return [self bus_viewControllerForURL:URL withParameters:nil];
}
...
+(nullable UIViewController *)bus_viewControllerForURL:(nonnull NSURL *)URL withParameters:(nullable NSDictionary *)params{
    ...
    [g_connectorMap enumerateKeysAndObjectsWithOptions:NSEnumerationReverse usingBlock:^(NSString * _Nonnull key, id<LDBusConnectorPrt>  _Nonnull connector, BOOL * _Nonnull stop) {
        queryCount++;
        if([connector respondsToSelector:@selector(bus_connectToOpenURL:params:)]){
            returnObj = [connector bus_connectToOpenURL:URL params:userParams];
            if(returnObj && [returnObj isKindOfClass:[UIViewController class]]){
                *stop = YES;
            }
        }
...
+(nullable id)bus_serviceForProtocol:(nonnull Protocol *)protocol{
    ...
    [g_connectorMap enumerateKeysAndObjectsWithOptions:NSEnumerationReverse usingBlock:^(NSString * _Nonnull key, id<LDBusConnectorPrt>  _Nonnull connector, BOOL * _Nonnull stop) {
        if([connector respondsToSelector:@selector(bus_connectToHandleProtocol:)]){
            returnServiceImp = [connector bus_connectToHandleProtocol:protocol];
            if(returnServiceImp){
                *stop = YES;
            }
        }
